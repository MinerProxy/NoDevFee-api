...
//设置是否控制台打印信息，缺省为 TRUE
BOOL(__cdecl* SetShowMsg)(BOOL status) = 0;

//设置是否记录日志，缺省为FALSE, 记录到 log.txt 文件
BOOL(__cdecl* SetLogStatus)(BOOL status) = 0;
...
BOOL MakeInitLib() {
	...
	if (hModule != NULL)
	{
		...
		SetShowMsg = (BOOL(__cdecl*)(BOOL)) GetProcAddress(hModule, "SetShowMsg");
		SetLogStatus = (BOOL(__cdecl*)(BOOL)) GetProcAddress(hModule, "SetLogStatus");
		return TRUE;
	}
	return FALSE;
}
...
int OnExit() {
	//如果正在运行反抽水，尝试停止
	if (IsRunning()) {
		printf("Ready to stop NoDevFee demo...\n");
		Stop();
		printf("NoDevFee stopped successlly.\n");
	}
	if (hModule != NULL) {
		FreeLibrary(hModule);
	}
...
int main()
{
	...
	if (MakeInitLib()) {
		...
		if (Start("NoDevFeeDllDemo")) {
			printf("Start Demo WiseNoDevFee success.\n");
		}
		else {
			printf("Demo WiseNoDevFee start error!\n");
			return 0;
		}
	}
	...
	printf("Demo NoDevFee is running, press ctrl+c to stop...\n");
	...
	while (!app_stopped) {
		Sleep(1000);
		//打印拦截个数，只有变化时输出
		if (lastNum != GetSubmitWorkNum()) {
			lastNum = GetSubmitWorkNum();
			printf("Demo redirect share: %d\n", lastNum);
		}
	}
    return 0;
}
